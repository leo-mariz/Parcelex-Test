...
abstract class IBiometricAuthService {
  ...
  /// Login por telefone/OTP: só marca que o usuário optou por biometria no app.
  Future<void> enableBiometricsWithoutStoredCredentials();
...
class BiometricAuthServiceImpl implements IBiometricAuthService{
  ...
  Future<bool> isBiometricsEnabled() async {
    try {
      final storedCredentials =
          await _secureStorage.read(key: SecureStorageKeys.credentials);
      if (storedCredentials != null) return true;
      final marker = await _secureStorage.read(
        key: SecureStorageKeys.phoneAuthBiometricMarker,
      );
      return marker == 'true';
    } catch (e) {
      return false;
  ...
  @override
  Future<void> enableBiometricsWithoutStoredCredentials() async {
    await _secureStorage.write(
      key: SecureStorageKeys.phoneAuthBiometricMarker,
      value: 'true',
    );
  ...
  Future<void> disableBiometrics() async {
    try {
      await _secureStorage.delete(key: SecureStorageKeys.credentials);
      await _secureStorage.delete(key: SecureStorageKeys.phoneAuthBiometricMarker);
    } catch (e) {
...
class SecureStorageKeys {
  ...
  /// Flag quando o login não usa e-mail/senha locais (ex.: só OTP).
  static const String phoneAuthBiometricMarker = 'phone_auth_biometric_marker';
}
